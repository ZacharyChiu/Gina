...
pygame.display.set_caption('ColorInPic')
...
class PicBar():
	def __init__(self,base,aalilh,n_w):
		...
		self.sur = []  # 所有surface对象。结构：[[主图一，色1，色2，……],[主图二，色1，色2，……], D:\\#My\\GiData\\Source\\Arts\\Images\\Pics.]
		self.ratos = []
		self.hs = []
		self.base = base
		self.aalilh = aalilh
...
all_pic_list = clean(os.listdir('D:\\#My\\GiData\\Source\\Arts\\Images\\Pics'))
pic_list = []  # 提取过颜色的图片（文件名）集合
for i in color_i:
	for p in all_pic_list:
		if '.'+i+'.' in p:
			pic_list.append('D:\\#My\\GiData\\Source\\Arts\\Images\\Pics\\'+p)
	...
	for event in pygame.event.get():
		...
		if event.type == pygame.MOUSEBUTTONDOWN:
			...
			if event.button == 4:
				...
				try:
					print('Index【'+bar.msin(scroll_length).split('.')[-2]+'】')
				except:
					pass
