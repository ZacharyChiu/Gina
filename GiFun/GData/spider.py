...
class Huaban():
	...
	def byboard(self,board_id,lenth=100):
		...
		if type(url) == list:
			...
			self.urlNext = url + self.buff
			self.requestUrl(url,lenth=lenth)
		return self.result
	...
	def download(self,lenth=1000,source='picurls.txt'):
		try:
			if os.path.exists(source):
				...
				for i in urls[:lenth]:
					...
					print('\r进度【' + perc + '】', end='')
					with open('download/' + str(urls.index(i)+1)+'.jpg','wb') as f:
						f.write(pic.content)
			else:
				print('没有资源文件')
		except Exception as e:
			print('PASS ONE PICTURE!!!')
			print(e)
...
class Baidu():
    def __init__(self):
        self.bd_url = 'https://www.baidu.com/s?&wd='
        self.baike_url = 'https://baike.baidu.com/item/'
        # self.url = url
        # self.page = self.get_page()
        
        
    def get_page(self,url,enco="utf-8"):
        head = {'User-Agent': 'Mozilla/5.0 (Windows; U; Windows NT 6.1; en-US; rv:1.9.1.6) Gecko/20091201 Firefox/3.5.6'}
        TimeOut = 30
        ans = []
		# Page = requests.session().get(url, headers=self.head, timeout=self.TimeOut)
        ...
        # article = self.html.find_all('article')
        # print(article)
    def search(self,q):
        blocks = self.get_page(self.bd_url+q+'&pn=1').find_all('div',id="content_left")[0]
        titles = []
        hrefs = []
        locations = []
        # for i in range(len(blocks)):
            # t = blocks[i].a.get_text()
            # h = blocks[i].a.attrs['href']
            # loc = blocks[i].find_all('div',class_="f13 c-gap-top-xsmall se_st_footer user-avatar")[0].a
            # titles.append(t)
            # hrefs.append(h)
    ...
    def baike(self,q):
        blocks = self.get_page(self.baike_url+q).find_all('div',class_='lemma-summary')
        introduction = blocks[0].find_all('div',class_='para')
        address = []    
        result = {}
        result['intro'] = ''
        for i in range(len(introduction)):
            blues = introduction[i].find_all('a')
            for j in blues:
                address.append(j)
            result['intro'] += introduction[i].get_text()+'\n'
...
if __name__ == '__main__':
	# hua = Huaban()
	# # 爬取某画板所有图片
	# result = hua.byboard('50386383',818)
	# with open('urls.txt','w',encoding='utf-8') as f:
		# f.write('\n'.join(result))
	# print('\n'.join(result))
	
	# 下载
	# hua.download(source='urls.txt')
