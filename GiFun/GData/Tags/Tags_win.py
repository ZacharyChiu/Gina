...
def andget(tags,v_path):
	...
	if f_list:
		for t in range(len(f_list)):	
			old_file_name = f_list[t]
			# print('进度【'+str(t+1)+'/'+str(len(f_list))+'】','| 提取：',old_file_name[41:])
			percent = int(((t+1)/len(f_list)*100)/5)
			perc = '★'*percent+'☆'*(20-percent)
			
			print('\r进度【'+perc+'】',end='')
			new_file_name = v_path+'\\'+f_list[t]
			os.system('copy '+old_file_name+' '+new_file_name)
	print('\n')
...
def replace(v_path,tag_txt):
	'''输入格式：旧标签.新标签'''
	o_n = tag_txt.split('.')
	old_tag = o_n[0]
	new_tag = o_n[1]
	c_name_list = clean(os.listdir(v_path))
	for c_name in c_name_list:	#所有需要编辑的文件名
		pre = ''
		build = ''
		goal = ''
		shell = win32com.client.Dispatch("WScript.Shell")
		path = os.getcwd() + '\\' + v_path+'\\' + c_name	#所有需要编辑的文件的路径
		shortcut = shell.CreateShortCut(path)
		target = shortcut.Targetpath	#溯源
		# print(target)
		path_name = target.split('\\')
		heads = path_name[:-1]
		for head in heads:
			pre = pre+head+'/'
		botton = path_name[-1]	#文件名
		parts = botton.split('.')	#拆分文件名
		if old_tag in parts:
			num = parts.index(old_tag)
			parts[num] = new_tag
			build = parts[0]
			for part in parts[1:]:
				build = build+'.'+part
...
def same(path1):
	...
	files = clean(os.listdir('\\'.join(os.getcwd().split('\\')[:-1])))
	for file in files:
		list1.append(file)
	...
	for n in range(len(list1)):
		hash = md5()
		img = open('\\'.join(os.getcwd().split('\\')[:-1])+'\\'+list1[n], 'rb')
	...
	while m < len(list):
		...
		while t < len(list):
			if list[m][1] == list[t][1]:
				if ddd == 0:
					os.remove('\\'.join(os.getcwd().split('\\')[:-1]) + '\\' + list[t][0])
				elif ddd == 1:
					o_path1 = '\\'.join(os.getcwd().split('\\')[:-1]) + '\\' + list[m][0]
					n_path1 = os.getcwd() + '\\' + path1+'\\'+list[m][0]
					o_path2 = '\\'.join(os.getcwd().split('\\')[:-1]) + '\\' + list[t][0]
...
def scan_all(flist,svPath='HSkeyLibrary.csv'):
	...
	for f in flist:
		index = f.split('.')[-2]
		key = get_md5('\\'.join(os.getcwd().split('\\')[:-1]) + '\\' + f)
...
def find_id(HS,flist):
	i = ''
	for f in flist:
		fullpath = '\\'.join(os.getcwd().split('\\')[:-1]) + '\\' + f
...
view_path1 = 'cash\\cash0'				#最终的浏览文件夹
view_path2 = 'cash\\cash1'
view_path3 = 'cash\\cash2'
view_path = view_path1
...
while mode != 'q':
	...
	try:
		if mode == 'test':
			...
			os.startfile('\\'.join(os.getcwd().split('\\')[:-1]))
		elif mode == 'opc':
			os.startfile(view_path)
		elif mode == 'tag':
			...
			for w in webp_list:
				if '[gif]' in w:
					...
					clean_list.append(w)
			full_path = '\\'.join(os.getcwd().split('\\')[:-1])
			m = input('1: 抽取webp静态图\n2: 抽取webp动态图\n选择：')
			...
			replace(view_path,tag_txt)
			os.startfile(view_path)
		
		
		elif mode == 'all':
			clear_cash(view_path)
			file_name_list = clean(os.listdir('\\'.join(os.getcwd().split('\\')[:-1])))#获取a文件夹内所有文件名
			for name in file_name_list:	
				if '.' in name:
					old_file_name = '\\'.join(os.getcwd().split('\\')[:-1]) + '\\' + name
			...
			os.system(r'start C:\MySoftware\Notepad++/notepad++.exe Tags_win.py')
		elif mode == 'path':
			os.startfile('')
		elif mode == 'pi':
			...
			file_name_list = clean(os.listdir('..'))#获取a文件夹内所有文件名
			flen = len(file_name_list)
			for fn in file_name_list:
				...
				newname = '..'+'\\'+'.'.join(ftaglist)+'.'+'.'.join(ftlist[-2:])
			...
			for fn in file_name_list:
				...
				newname = '..'+'\\'+'.'.join(ftaglist[:-2])+'.'+transnum(num+1)+'.'+ftaglist[-1]
				# print('newname:',newname)
				os.rename('..'+"\\"+fn,newname)
				percent = int(((num+1)/len(file_name_list)*100)/5)
				perc = '★'*percent+'☆'*(20-percent)
			...
			for fn in file_name_list:
				...
				f3 = ftaglist[-3]
				num = file_name_list.index(fn)
				newname = '..'+'\\'+'.'.join(ftaglist[:-2])+'.'+str(num+1)+'.'+ftaglist[-1]
				# print('newname:',newname)
				os.rename('..'+"\\"+fn,newname)
				percent = int(((num+1)/len(file_name_list)*100)/5)
				perc = '★'*percent+'☆'*(20-percent)
			...
			for fn in file_name_list:
				try:
					img = Image.open('\\'.join(os.getcwd().split('\\')[:-1])+'\\'+fn)
					size = img.size
					if tag_txt == 'o':
			...
			print('*'*50)
			for tg in target:
				old_file_name = '\\'.join(os.getcwd().split('\\')[:-1]) + '\\' + tg
			...
			print('\n\n')
			os.system('python Tags_Win.py')
			
		elif mode == 'rd':
			clear_cash(view_path)
			file_name_list = clean(os.listdir('..'))#获取源文件夹内所有文件名
			randindex = random.randint(0,len(file_name_list))
			while randindex in drawed:
				randindex = random.randint(0,len(file_name_list))
			print('已排除画过的项目：\n',drawed)
			print('抽取：',randindex,'号')
			addtag(str(randindex),view_path)
			os.startfile(view_path)
		elif mode == 'cs':
			if tag_txt == 'un':
		...
		## 关联文件·专题 ##
			...
			for f in f_list:
				if f.split('.')[-2] == index:
					...
					o_file = f
			if o_file != '':
				key = get_md5('\\'.join(os.getcwd().split('\\')[:-1]) + '\\' + o_file)
